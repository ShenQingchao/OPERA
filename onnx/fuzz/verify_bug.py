    ...
    for key, value in kwargs.items():
        if isinstance(value, str) and value.startswith('['):
            try:
                ...
                #value_np = np.array(eval(value))
                new_kwargs[key] = onnx.numpy_helper.from_array(value_np)
            except Exception as e:
                print(f"Failed to convert value for key {key}: {e}")
...
def assign_input_data(shape, dtype):    
    ...
    if dtype == onnx.TensorProto.FLOAT:
        ...
        return np.random.randint(0, 3, shape, dtype=np.int16)
    elif dtype == onnx.TensorProto.INT8:
        return np.random.randint(0, 3, shape, dtype=np.int8)
    elif dtype == onnx.TensorProto.UINT8:
        return np.random.randint(-3, 3, shape, dtype=np.uint8)
    elif dtype == onnx.TensorProto.UINT16:
        return np.random.randint(-3, 3, shape, dtype=np.uint16)
    elif dtype == onnx.TensorProto.UINT32:
        return np.random.randint(-3, 3, shape, dtype=np.uint32)
    elif dtype == onnx.TensorProto.UINT64:
        return np.random.randint(-3, 3, shape, dtype=np.uint64)
...
def make_graph(op_type, kwargs, input_name, input_shape, input_dtype, output_name, output_shape, output_dtype, count=0, **unused_kwargs):
    ...
    if True:
        ...
        for name, shape, dtype in zip(input_name, input_shape, input_dtype_onnx):
            ...
            #elif op_type.startswith("Sequence"):
            #    input_data[name] = onnx.numpy_helper.from_list(input_data[name])
        print(">>>>>>>input_data")
        print(input_data)
        ...
        # Print the ONNX runtime output
        print('The ONNX runtime output is:\n{}'.format(onnx_output))
...
def compile_onnx(model, input_name, input_shape, input_data, input_dtype, output_name, exec_mod='graph'):
    ...
    print(">>>>> test_x_tvm:", test_x_tvm)
    print(">>>>> tvm_out_np:", tvm_out_np)
    return tvm_out_np

make_graph(op_type='Slice', kwargs={}, input_name=('x', 'starts', 'ends', 'axes', 'steps'), input_shape=([20, 10, 5], [2], [2], [2], [2]), input_dtype=('FLOAT', 'INT64', 'INT64', 'INT64', 'INT64'), output_name=('y',), output_shape=([3, 10, 5],), output_dtype=('FLOAT',),count=34,)
