...
def get_abstract_list(_list):
    ...
    # 0 for equal, 1 for greater, -1 for less or not equal (for list/tuple), None for not number
    abstract_list = []
    for i, e in enumerate(_list):
        if e is None:
            ...
            abstract_list.append(e.decode())
        elif isinstance(e, float):
            new_v = 0.1 if e > 0 else -0.1  # define the default abstract value for floats
            abstract_list.append(new_v)
        else:
            raise TypeError(f"Unsupported type {type(e)} in {_list}!")

    if len(abstract_list) == 2:
        e0 = abstract_list[0]
        e1 = abstract_list[1]
        if e0 == e1:
