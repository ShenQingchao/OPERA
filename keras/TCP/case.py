...
class TC:
    ...
    def parse_test(self):
        ...
        if input_str.find('input_dtype') == -1:
            input_shape = eval(input_str.strip()[:-2])
        ...
        print(args_kwargs)
        if not args_kwargs.startswith('args'):
            kwargs = args_kwargs[len("kwargs="):]
            print(kwargs)
        else:
        ...
        if is_correct_api(self.layer):
            ...
            # else:
            #     print('warning: ', self.all_para_info)
            #     self.is_valid = False
    ...
    def parse_torch_test(self):
        # layer_test(keras.layers.Dense,args=(),kwargs={'units':32,},input_shape=[3, 784],input_dtype='float32',)
        print(f'{"-" * 20}\n>>> Test case {self.id}: ', self.test_cmd_str.strip())
        layer_args, input_str = self.test_cmd_str.split(",.eval(), input_data=")
        input_shape, input_dtype = input_str.strip()[:-2].split(",input_dtype=")
        input_shape = eval(input_shape)
        input_dtype = eval(input_dtype)  # delete the ''
        self.all_para_info['input_shape'] = input_shape
    ...
    def get_abstract_test_case(_dict):
        ...
        for k, v in _dict.items():
            ...
            elif isinstance(v, bool) or isinstance(v, str) or isinstance(v, complex):
                new_v = v
...
class TCDict:
    ...
    def select_instance(self, layer_name, SUT):
        ...
        random.shuffle(candidate_instance_list)
        max_distance = 0
    ...
    def calc_distance(this_tc_dict: dict, selected_tc_dict: dict, selected_tc_pair: dict, SUT):
        ...
        if len(this_tc_dict) == 1 and 'input_shape' in this_tc_dict:
            if this_tc_dict['input_shape'] not in selected_tc_dict['input_shape']:
                distance = 1.0
            else:
                distance = 0.0
