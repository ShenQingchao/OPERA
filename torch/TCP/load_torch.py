...
import inspect
import torch
from TCP import utils, case
from TCP.case import TC, TCDict
from TCP.utils import is_correct_api
...
def parse_pytorch_func(test_cmd_str):
    ...
    for line in lines:
        ...
        line = line.replace(').bool(', ', dtype=torch.bool')
    ...
    elif 'input_dtype' in all_para_info and all_para_info['input_dtype'].find('q') != -1:
        return "Invalid", "no_default"
...
def parse_torch_cls(test_cmd_str):
    ...
    test_cmd_str = test_cmd_str.replace(').bool()', ', dtype=torch.bool)')
    all_para_info = {}
...
def preprocess_torch_test(test_file):
    ...
    tc_dict = TCDict()
    print(test_file, len(all_test_cases))
    for tc_id, test_case in enumerate(all_test_cases):
        # if tc_id >= len(all_test_cases) / 2:
        #     continue
        is_valid = False
        ...
        if is_correct_api(layer):
            ...
            if processed_args:  # return False if lack value for an un-default para.
                ...
                is_valid = True
                new_tc = TC(tc_id+1, test_cmd_str=test_case + '\n', layer=layer, all_para_info=all_para_info,
...
def preprocess_params(default_dict: dict, collected_dict: dict):
    ...
    if 'input' in result_dict:
        if result_dict['input'] == 'no_default':
            result_dict['input'] = ''
